...
contract HomeChainProduction
{
    ...
    function getPricesSaleOfEstate(uint id) public view returns (uint)
    {
        return pricesSale[id];
    }

    function getPricesRentOfEstate(uint id) public view returns (uint)
    {
        return pricesRent[id];
    }

    function getAddressOfApprovedBuyer(uint id) public view returns (address payable)
    {
        return buyers[id];
    }

    function getAddressOfApprovedTenant(uint id) public view returns (address payable)
    {
        return tenants[id];
    }
